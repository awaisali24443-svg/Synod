  ...
  return (
    <div className="bg-synod-bg-secondary border border-white/10 rounded-xl p-4 flex flex-col space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold uppercase tracking-wider text-gray-400">Control Panel</h2>
      ...
      <div className="space-y-3">
        <div>
          <label className="block text-xs text-gray-500 mb-1 font-mono">TARGET DOMAIN</label>
          <input 
            type="text" 
            value={activeTargetDomain}
            onChange={(e) => setActiveTargetDomain(e.target.value)}
            className="w-full bg-black/50 border border-white/10 rounded px-3 py-2 text-sm font-mono text-synod-accent focus:outline-none focus:border-synod-accent/50 transition-colors"
            placeholder="example.com"
          />
        </div>

        <div>
          <label className="block text-xs text-gray-500 mb-1 font-mono">OPERATION MODE</label>
              ...
              <button
                ...
                className={cn(
                  ...
                  scanMode === mode.id 
                    ...
                    : "bg-black/30 border-white/5 text-gray-500 hover:bg-white/5 hover:text-gray-300"
                )}
              >
        ...
        <form onSubmit={handleExecute} className="pt-2">
          <label className="block text-xs text-gray-500 mb-1 font-mono">MANUAL OVERRIDE</label>
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-black/50 border border-white/10 rounded pl-9 pr-10 py-2 text-sm font-mono text-white focus:outline-none focus:border-synod-ai/50 transition-colors"
